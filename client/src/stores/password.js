            ...
            api.post(`${this.url}`, this.item)
                ...
                .catch(err => {
                    ...
                    if (err.message === "Title must be only letters.") {
                        app.$toast.error("Полето заглавие трябва да съдържа само главни, малки букви и интервали.", { duration: 10000 });
                    }
                    if (err.message === "Title must be longer than 3 letters.") {
                        app.$toast.error("Полето заглавие трябва да съдържа повече от 3 букви.", { duration: 7000 });
                    }
                    if (err.message === "Password is required!") {
                        app.$toast.error("Полето парола е задължително!", { duration: 4000 });
                    }
                })
